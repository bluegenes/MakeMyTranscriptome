...
def gen_expression_supervisor(fastq1,fastq2,paired_names,unpaired,unpaired_names,cpu,sample_info,model,dependency_set,run_intersectbed=False,run_express=False, assembly_name=tf.NAME_ASSEMBLY, assembly_path= tf.GEN_PATH_ASSEMBLY(), out_dir=tf.GEN_PATH_EXPRESSION_FILES()):
    fasta_to_bed = tf.assembly_to_bed_task(assembly_path, assembly_name, out_dir,[])
    build_salmon = tf.build_salmon_task(assembly_path, assembly_name, out_dir,cpu,[])
    gene_trans_map = tf.gene_trans_map_task(assembly_path,assembly_name, out_dir,[]) # since out_dir is no longer annot_dir, this task will repeat for main assembly
    salmon_gene_map = tf.salmon_gene_map_task(out_dir,assembly_name,gene_trans_map.targets[0],[gene_trans_map])
    build_bowtie = tf.build_bowtie_task(assembly_path,assembly_name, out_dir,[])
    ...
    for i in range(len(fastq1)):
        ...
        salmon = tf.salmon_task(build_salmon.targets[0],fastq1[i],fastq2[i],paired_names[i]+'_salmon', salmon_gene_map.targets[0],out_dir,int(cpu/2),[build_salmon, salmon_gene_map])
    ...
    for i in range(len(unpaired)):
        ...
        salmon = tf.salmon_unpaired_task(build_salmon.targets[0],unpaired_names[i],unpaired_names[i]+'_salmon',salmon_gene_map.targets[0],out_dir,int(cpu/2),[build_salmon,salmon_gene_map])
        salmon_tasks.append(salmon)
        if run_express:
            bowtie_e_tasks.append(bowtie_e)
            express_tasks.append(express)
        if run_intersectbed:
            ...
            sam_sort_tasks.append(sam_sort)
            intersect_tasks.append(intersect_bed)
    counts_to_table_express = tf.counts_to_table_task(gene_trans_map.targets[0],out_dir, [t.targets[0] for t in express_tasks],'express_counts','--eXpress',express_tasks+[gene_trans_map])
    counts_to_table_intersect = tf.counts_to_table_task(gene_trans_map.targets[0],out_dir,[t.targets[0] for t in intersect_tasks],'bed_counts','',intersect_tasks+[gene_trans_map])
    counts_to_table_salmon = tf.counts_to_table_task(gene_trans_map.targets[0],out_dir, [t.targets[0] for t in salmon_tasks],'salmon_counts','--salmon',salmon_tasks+[gene_trans_map])
    deseq2_express = tf.deseq2_task(out_dir,counts_to_table_express.targets[0],sample_info,'express',model,[counts_to_table_express])
    deseq2_intersect = tf.deseq2_task(out_dir,counts_to_table_intersect.targets[0],sample_info,'intersect',model,[counts_to_table_intersect])
    deseq2_salmon = tf.deseq2_task(out_dir,counts_to_table_salmon.targets[0],sample_info,'salmon',model,[counts_to_table_salmon])
