...
=======
def gen_expression_supervisor(fastq1,fastq2,paired_names,unpaired,unpaired_names,cpu,sample_info,model,gene_trans_map,dependency_set,run_intersectbed=False,run_express=False, assembly_name=fg.NAME_ASSEMBLY, assembly_path= fg.GEN_PATH_ASSEMBLY(), out_dir=fg.GEN_PATH_EXPRESSION_FILES()):
>>>>>>> 3583cab2e0ae3fda815a132d72b680c8b6eb9990
...
>>>>>>> 3583cab2e0ae3fda815a132d72b680c8b6eb9990
    ...
    for i in range(len(unpaired)):
        salmon = fex.salmon_unpaired_task(build_salmon.targets[0],unpaired[i],unpaired_names[i]+'_salmon',salmon_gene_map.targets[0],out_dir,int(cpu/2),[build_salmon,salmon_gene_map])
        salmon_tasks.append(salmon)
        if run_express:
        ...
        if run_intersectbed:
            ...
            intersect_tasks.append(intersect_bed)
    counts_to_table_salmon=fex.counts_to_table_task(assembly_name, gene_trans_map,out_dir, [t.targets[0] for t in salmon_tasks],'salmon_counts','--salmon',salmon_tasks)
    deseq2_salmon = fex.deseq2_task(assembly_name,out_dir,counts_to_table_salmon.targets[0],sample_info,'salmon',model,[counts_to_table_salmon])
    deseq2_salmon_gene = fex.deseq2_task(assembly_name,out_dir,counts_to_table_salmon.targets[1],sample_info,'salmon_gene',model,[counts_to_table_salmon])
    ...
    if run_express or run_intersectbed:
        ...
        if run_express:
            counts_to_table_express = fex.counts_to_table_task(assembly_name,gene_trans_map,out_dir, [t.targets[0] for t in express_tasks],'express_counts','--eXpress',express_tasks)
            deseq2_express = fex.deseq2_task(assembly_name,out_dir,counts_to_table_express.targets[0],sample_info,'express',model,[counts_to_table_express])
            deseq2_express_gene = fex.deseq2_task(assembly_name,out_dir,counts_to_table_express.targets[1],sample_info,'express_gene',model,[counts_to_table_express])
            all_tasks.extend([counts_to_table_express,deseq2_express, deseq2_express_gene]+bowtie_e_tasks+express_tasks)
        if run_intersectbed:
            counts_to_table_intersect=fex.counts_to_table_task(assembly_name,gene_trans_map,out_dir,[t.targets[0] for t in intersect_tasks],'bed_counts','',intersect_tasks)
            deseq2_intersect = fex.deseq2_task(assembly_name,out_dir,counts_to_table_intersect.targets[0],sample_info,'intersect',model,[counts_to_table_intersect])
            deseq2_intersect_gene = fex.deseq2_task(assembly_name,out_dir,counts_to_table_intersect.targets[1],sample_info,'intersect_gene',model,[counts_to_table_intersect])
