...
def gen_expression_supervisor(fastq1,fastq2,paired_names,unpaired,unpaired_names,cpu,sample_info,model,dependency_set,run_intersectbed=False,run_express=False):
    fasta_to_bed = tf.assembly_to_bed_task([])
    salmon_tasks = []
    build_salmon = tf.build_salmon_task(cpu,[])
    gene_trans_map = tf.gene_trans_map_task([])
    salmon_gene_map = tf.salmon_gene_map_task(gene_trans_map.targets[0],[gene_trans_map])
    ...
    counts_to_table_salmon = tf.counts_to_table_task([t.targets[0] for t in salmon_tasks],'salmon_counts','--salmon',salmon_tasks)
	...
	if run_express:
        ...
        if run_intersectbed:
            all_tasks = all_tasks.extend([counts_to_table_intersect,deseq2_intersect]+bowtie_i_tasks+sam_sort_tasks+intersect_tasks)
